...
char * get_IP_from_hostname(char * hostname) {
    ...
    struct hostent * target_hostname = gethostbyname(hostname);
    if(target_hostname){
        ...
        in_addr * ip_address = (in_addr *)target_hostname->h_addr;
        return inet_ntoa(*ip_address); //dereferencing ip_address pointer
    }
